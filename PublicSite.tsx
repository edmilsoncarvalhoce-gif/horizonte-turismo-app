...
export const PublicSite: React.FC<PublicSiteProps> = ({ packages, onBook, onAddReview }) => {
  ...
  const openBookingModal = (pkg: TravelPackage) => {
    // If opening from details modal, keep pkg selected, just close details and open booking
    setSelectedPackage(pkg);
    setIsDetailsModalOpen(false);
    
    // Auto-fill logic for date
    let initialDate = '';
  ...
  const handleSubmitReview = (e: React.FormEvent) => {
    ...
    if (selectedPackage && reviewForm.comment && reviewForm.userName) {
        ...
        setReviewForm({ userName: '', rating: 5, comment: '' });
        // Normally we might show a success toast here
    }
  };
          ...
          <div className="bg-white p-4 rounded-2xl shadow-xl max-w-4xl flex flex-col md:flex-row gap-4">
            <div className="flex-1 flex items-center gap-3 px-4 border-b md:border-b-0 md:border-r border-slate-200 pb-4 md:pb-0">
              ...
              <div>
                ...
                <input type="text" placeholder="Qual seu destino?" className="w-full outline-none text-slate-800 placeholder-slate-400" />
              </div>
            </div>
            <div className="flex-1 flex items-center gap-3 px-4 border-b md:border-b-0 md:border-r border-slate-200 pb-4 md:pb-0">
              ...
              <div>
                ...
                <input type="text" placeholder="Adicionar datas" className="w-full outline-none text-slate-800 placeholder-slate-400" />
              </div>
            </div>
                        ...
                        <div className="relative">
                          ...
                          {selectedPackage.availableDates && selectedPackage.availableDates.length > 0 ? (
                              selectedPackage.availableDates.length === 1 ? (
                                  // Case 1: Single Date (Read Only)
                                  <input 
                                    type="text" 
                                    disabled
                                    value={selectedPackage.availableDates[0]} // Display format logic could go here
                                  ...
                                  // Case 2: Multiple Dates (Dropdown)
                                  <div className="relative">
                                    <select 
                                        required
                                        value={bookingData.date}
                                        onChange={(e) => setBookingData({...bookingData, date: e.target.value})}
                                        className="w-full pl-9 pr-8 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 outline-none appearance-none"
                                    >
                              ...
                              // Case 3: No specific dates (Date Picker)
                              <input 
                                type="date" 
                                required
                                value={bookingData.date}
                        ...
                        <div>
                           ...
                           <div className="relative">
                              ...
                              <input 
                                ...
                                className="w-full pl-9 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 outline-none" 
                              />
                           </div>
                        </div>
                        <div>
